...
| FIX-05 | Fixbug | Admin class deletion with audited empty-class delete and history-preserving deactivation | `rpc_delete_class`, `class-detail.component.ts`, migration `202609060002` | Angular build; DB-T18 smoke test | IMPLEMENTED |
| FIX-06 | Fixbug | Admin sửa lịch học theo phiên bản, giữ nguyên lịch sử buổi đã sinh và audit thay đổi | `rpc_save_class_schedule`, `class-schedule.component.ts`, migration `202609060003` | Deno check; remote RPC smoke | IMPLEMENTED |
| FIX-07 | Fixbug | Admin chỉnh đơn giá học phí riêng theo enrollment, giữ nguyên ledger lịch sử và audit thay đổi | `rpc_update_enrollment_unit_price`, class/student detail, migration `202609060004` | Angular unit/build; Deno check; DB/RLS smoke pending | IMPLEMENTED |
| REBUILD-01 | Workflow hub | Role-based home/month/education/teaching/finance/people/notification/settings hubs | `app.routes.ts`, `app-shell.component.ts`, feature hub components | npm test/build | IMPLEMENTED |
| REBUILD-02 | Month setup | Browser draft with atomic period/snapshot/roster/schedule/session/assignment/carry-over creation | `month-setup.component.ts`, `create-month-setup`, `rpc_create_month_setup` | npm build; DB pending | IMPLEMENTED |
| REBUILD-03 | Work attendance | Per-session check-in/out and Admin review state machine | `staff_work_attendance`, submit/review Edge Functions and RPCs | Deno check; DB/RLS pending | IMPLEMENTED |
| REBUILD-04 | Availability | Staff day/time availability upsert with audit | `staff_availability`, `save-staff-availability` | Deno check; DB/RLS pending | IMPLEMENTED |
| REBUILD-05 | Notifications | Recipient inbox, unread/read, manual fan-out and automatic operational alerts | `notifications`, notification RPCs, `notifications.component.ts` | npm test/build; DB/RLS pending | IMPLEMENTED |
| REBUILD-06 | Snapshot payroll/tuition/close | New periods use snapshot fees and approved work attendance; close blocks unresolved work | migration overrides, `calculate-payroll`, `close-period-preview` | Deno check; DB pending | IMPLEMENTED |
| REBUILD-07 | Teaching scope | Teacher/assistant “Lớp của tôi” with assigned classes, schedules, active students, attendance history and evaluation comments | `my-classes.service.ts`, `my-classes.component.ts`, `my-class-detail.component.ts`, `/my-classes` | navigation/route tests; npm build | IMPLEMENTED |
