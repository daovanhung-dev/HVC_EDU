...
@Component({
  ...
  template: `
    ...
    <section class="kpi-grid"><article class="card"><span>Phải thu</span><strong>{{money(totals().total_due)}}</strong></article><article class="card"><span>Đã thu</span><strong>{{money(totals().total_paid)}}</strong></article><article class="card"><span>Công nợ</span><strong>{{money(totals().total_debt)}}</strong></article><article class="card"><span>Tỷ lệ thu</span><strong>{{(rate()*100).toFixed(1)}}%</strong></article></section>
