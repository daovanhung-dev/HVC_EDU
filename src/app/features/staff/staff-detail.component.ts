...
@Component({
  ...
  template: `
    ...
    <section class="card section-card"><h2>Thông tin</h2><div class="stat-list"><div><span>Loại</span><strong>{{staff()?.staff_type}}</strong></div><div><span>Môn</span><strong>{{staff()?.primary_subject||'—'}}</strong></div><div><span>Điện thoại</span><strong>{{staff()?.phone||'—'}}</strong></div><div><span>Trạng thái nhân sự</span><strong>{{staff()?.status}}</strong></div></div></section>
