...
type Item = { enrollment_id?: string; comment?: string | null };
export default { fetch: withSupabase({ auth: 'user' }, async (request, ctx: any) => {
  ...
  try {
    ...
    const items = body.rows.map((item) => ({ enrollment_id: requiredUuid(item.enrollment_id), comment: item.comment ?? null }));
    return finish(request, ok(await callRpc(ctx, 'rpc_upsert_evaluations', { p_session_id: sessionId, p_items: items, p_trace_id: traceId }), traceId));
